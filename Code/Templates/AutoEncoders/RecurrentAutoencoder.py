...
for rec in record:
    ...
    if ((len(data) + len(test)) % 6) == 5:
        test.append([rec.seq[i] for i in range(len(rec.seq))] + ['O' for _ in range(50 - len(rec.seq))])
    else:
        data.append([rec.seq[i] for i in range(len(rec.seq))] + ['O' for _ in range(50 - len(rec.seq))])

X = np.zeros((len(data), 50, 4))

for i, sentence in enumerate(data):
    X[i] = ctable.encode(sentence, maxlen=50)

X_val = np.zeros((len(test), 50, 4))

for i, sentence in enumerate(test):
    X_val[i] = ctable.encode(sentence, maxlen=50)

print("Creating model...")
model = Sequential()

#Recurrent encoder
model.add(recurrent.LSTM(encoding_dim, input_shape=(50, ACIDS), dropout_W=0.1, dropout_U=0.1))
