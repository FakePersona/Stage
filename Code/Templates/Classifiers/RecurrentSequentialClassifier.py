...
data = [2 * [''.join(np.random.choice(list(chars))) for _ in range(5)] for _ in range(120000)] + [[''.join(np.random.choice(list(chars))) for _ in range(10)] for _ in range(80000)]
...
test = [2 * [''.join(np.random.choice(list(chars))) for _ in range(5)] for _ in range(1000)] + [[''.join(np.random.choice(list(chars))) for _ in range(10)] for _ in range(1000)]
...
Conv.add(recurrent.LSTM(10, init='he_uniform', input_shape=(10, 26), return_sequences=True))

Conv.add(Flatten())
Conv.add(Dense(1, init='he_uniform'))
Conv.add(Activation('sigmoid'))

Conv.compile(optimizer='rmsprop', loss='binary_crossentropy', metrics=['accuracy'])
