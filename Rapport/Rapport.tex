...
(\cite{Spencer:2015:DLN:2817095.2817106}).

L'applications de techniques dévelloppées en langage naturels ou dans d'autres
...
paramètres nécessaires à la résolution du problème n'ait été calculés par
...
Une partie de notre travail s'est focalisé sur l'étude de ces représentations
...
\texttt{feature maps} permet de distinguer plus de points caractéristiques sui
...
L'idée est que l'entrée est traitée comme une suite de morceaux d'entrée:
typiquement c'est le cas d'une phrase par exemple. Chaque mot est traité par la
courche récurrente qui va calculer deux choses à partir de cette entrée: une
...
fait par un nombre de couche caché égal au nombre d'éléments dans la suite!
...
travers de quelquechose d'aussi simple que la descente de gradient, mais cette
dernière à de nombreux défauts. Dans cette article nous utiliserons les
...
\end{document}ne à effectuer des opérations linéaires (sommes pondérées
